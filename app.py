...
# =========================================================
    ...
    st.write("**KPIs actuales vinculados:**", kpis_puesto["indicador"].tolist())
    
    nuevo = st.text_input("NUEVO KPI").upper()
    if st.button("AGREGAR"):
        if nuevo:
            c.execute("INSERT INTO cargos VALUES (?,?)", (cargo_sel, nuevo))
            conn.commit()
            st.success(f"¡Indicador {nuevo} agregado con éxito a {cargo_sel}!")
            st.rerun()
        else:
